...
export function body(p: TemplateParams): string {
  return `\\begin{document}\n\n\\begin{center}\n  {\\LARGE \\textbf{\\color{primarycolor}${p.name}}}\\\\\n  ${p.title ? `\\color{secondarycolor}${p.title} \\` : ''}\n  ${p.coloredFullContactLine ? `${p.coloredFullContactLine} \\` : ''}\n\\end{center}\n\n${p.summary ? `\\vspace{${p.sectionSpacingSmall}mm}\\noindent \\color{primarycolor}${p.summary} \\vspace{${p.sectionSpacingMedium}mm}` : ''}\n\n${p.experiences ? `\\section*{\\color{primarycolor}Experience}\n${p.experiences}` : ''}\n\n${p.education ? `\\section*{\\color{primarycolor}Education}\n${p.education}` : ''}\n\n${p.skillsList ? `\\section*{\\color{primarycolor}Skills}\n\\color{primarycolor}${p.skillsList}` : ''}\n\n${p.linksSection ? `\\section*{\\color{primarycolor}Links}\n${p.linksSection}` : ''}\n\n${p.customSectionsTex}`;
}
